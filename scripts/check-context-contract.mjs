...
const metadataPaths = [
  resolve("examples/fixture-metadata.json"),
  resolve("examples/fixture-metadata-v2.json"),
];
...
for (const metadataPath of metadataPaths) {
  ...
  if (
    metadata.producerArtifact.startsWith("/") ||
    metadata.producerArtifact.split("/").includes("..")
  ) {
    throw new Error(
      "producerArtifact must be a repository-relative path without traversal.",
    );
  }
  ...
  if (fixture.format !== metadata.format) {
    throw new Error(
      `Fixture format ${fixture.format ?? "missing"} does not match ${metadata.format}.`,
    );
  }
  const actualHash = sha256(fixtureBytes);
  if (actualHash !== metadata.fixtureSha256) {
    throw new Error(
      `Fixture SHA-256 drifted: expected ${metadata.fixtureSha256}, received ${actualHash}.`,
    );
  }
  ...
  if (producerRoot) {
    const { stdout } = await run(
      ...
      [
        "-C",
        resolve(producerRoot),
        "show",
        `${metadata.producerCommit}:${metadata.producerArtifact}`,
      ],
    ...
    if (!producerBytes.equals(fixtureBytes)) {
      throw new Error(
        "Frozen fixture differs from the artifact at the declared producer commit.",
      );
    }
  }
